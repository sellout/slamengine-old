        ...
        letOne('tmp0,
               let(Map('left1 -> let(Map('left3 -> read("foo"),
                                   ...
                                   join(free('left3), free('right4),
                                        JoinType.Inner, JoinRel.Eq,
                                        ObjectProject(free('left3), constant(Data.Str("id"))),
                                        ObjectProject(free('right4), constant(Data.Str("foo_id"))))),
                       'right2 -> read("baz")),
                 join(free('left1), free('right2),
                      JoinType.Inner, JoinRel.Eq,
                      ObjectProject(free('right2), constant(Data.Str("bar_id"))),
                      ObjectProject(
                        ObjectProject(free('left1), constant(Data.Str("right"))),
